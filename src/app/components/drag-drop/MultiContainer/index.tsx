...
export function MultiContainer() {
        ...
        async function loadData() {
            ...
            if (containersRes.ok) {
                ...
                nextContainerIndex.current =
                    (data.containerOrder ? data.containerOrder.length : Object.keys(data.containers || {}).length) + 1
            }
    ...
    const findContainerOf = (itemId: string): string | null => {
        return (
            Object.entries(containers).find(([, items]) => items.includes(itemId))?.[0] ?? null
        )
    }
                                ...
                                <div className="space-y-2 mt-2">
                                    {items[containerId]?.map((mini) => (
                                        <DraggableItem
                                            ...
                                            id={mini.id}
                                            name={mini.name}
                                            onDelete={(id: string): void => handleDeleteItem(containerId, id)}
                                        />
                                    ))}
